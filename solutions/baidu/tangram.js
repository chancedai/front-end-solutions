var T, baidu = T = function() {
    ...
    function() {
                    ...
                    if (prev !== '*' && prev !== curr) {
                        ...
                        if (!conv) {
                            for (var key in converters) {
                                array = key.split(' ');
                                if (array[1] === curr) {
                                    conv = converters[prev + ' ' + array[0]] || converters['* ' + array[0]];
                                    if (conv) {
                                        if (conv === true) {
                                            conv = converters[key];
                        ...
                        if (conv !== true) {
                            if (conv && opts['throws']) {
                                ...
                                try {
                                    ...
                                    return {
                                        state: 'parsererror',
                                        error: conv ? e : 'No conversion from ' + prev + ' to ' + curr
                                    };
                                }
                            }
                        }
        ...
        baidu.createChain('ajax', function(url, options) {
            ...
            if (opts.crossDomain == null) {
                ...
                opts.crossDomain = !!(parts && (parts[1] != ajaxLocParts[1] || parts[2] != ajaxLocParts[2] || (parts[3] || (parts[1] === 'http:' ? 80 : 443)) !=
            ...
            if (opts.data && opts.hasContent && opts.contentType !== false || options.contentType) {
                tangXHR.setRequestHeader('Content-Type', opts.contentType);
            }
            if (opts.ifModified) {
                ifModifiedKey = ifModifiedKey || opts.url;
                lastModified[ifModifiedKey] && tangXHR.setRequestHeader('If-Modified-Since', lastModified[ifModifiedKey]);
                etag[ifModifiedKey] && tangXHR.setRequestHeader('If-None-Match', etag[ifModifiedKey]);
            }

            tangXHR.setRequestHeader('Accept',
                opts.dataTypes[0] && opts.accepts[opts.dataTypes[0]] ?
                opts.accepts[opts.dataTypes[0]] + (opts.dataTypes[0] !== '*' ? ', ' + allTypes + '; q=0.01' : '') : opts.accepts['*']);
        ...
        baidu.ajax.prefilter('json jsonp', function(opts, originalSettings, tangXHR) {
            var callbackName, overwritten, responseContainer,
                ...
                && !(opts.contentType || '').indexOf('application/x-www-form-urlencoded') && rjsonp.test(data);
    ...
    baidu._util_.smartScroll = function(axis) {
        ...
        function getWindow(ele) {
            return baidu.type(ele) == "Window" ? ele : isDocument(ele) ? ele.defaultView || ele.parentWindow : false;
        }
        return {
            get: function(ele) {
    ...
    baidu.fx.create = function(element, options, fxName) {
        ...
        timeline.addEventListener("onbeforestart", function() {
            ...
            if (!me.overlapping) {
                (guid = me.element.getAttribute(me.attribName)) && baiduInstance(guid).cancel();
